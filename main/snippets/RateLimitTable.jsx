...
// Accepts tier + optional api prop. When api is provided renders only that API's tables.
export function RateLimitTable({ tier, api }) {
    ...
    <>
      {apisToRender.map((apiData) => (
        <div key={apiData.name}>
          {apiData.globalLimits && apiData.globalLimits.length > 0 && (
            <>
              <p style={{ fontSize: '14px', color: '#6B7280', marginBottom: '8px' }}>
                Global limit — applies to all endpoints unless a specific endpoint policy overrides it.
              </p>
              <GlobalLimitsTable limits={apiData.globalLimits} />
            </>
          )}
